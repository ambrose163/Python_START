...
async def hello(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(emoji.emojize(':waving_hand:'))
    await update.message.reply_text(f'Привет, {update.effective_user.first_name}!\n'
                                    '\nЭто бот RLE шифрования строки.')

    time.sleep(5)
    await update.message.reply_text(f'Введи символы после команды /rle '
...
async def rle_code(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(emoji.emojize(':check_mark_button:'))
